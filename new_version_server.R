...
observeEvent(input$run_process, {
  ...
  # --- NEW: use the selected name column ---
  req(input$heatmap_name_col)
  name_col <- input$heatmap_name_col
  
  # Put the selected name column first and give it a consistent name
  ...
  # Apply the `remove_ether_lipids` function to remove ether lipids (O- or P- prefixed lipids)
  ...
  observeEvent(input$run_process, {
    ...
    output$modal_download_heatmap <- downloadHandler(
      filename = function() {
        paste0("heatmap_plot_", Sys.Date(), ".", input$modal_image_format)
      },
      content = function(file) {
        ...
        ggsave(
          ...
          width    = dims$width_in,     # in inches, same as app
          height   = dims$height_in,    # in inches, same as app
          units    = "in",
          dpi      = dpi                # higher dpi → more pixels, same layout
        )
