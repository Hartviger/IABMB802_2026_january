...
data_processed <- eventReactive(input$process, {
  ...
  non_samples <- df[!(stype %in% c("Blank", "QC", "Sample")), , drop = FALSE]  # currently unused, but kept for future logic
  ...
  new_QC_names_base      <- paste0(existing_qcs[[name_col]], "_QC")
  new_QC_samples_names   <- paste0(existing_qcs[[name_col]], "_QC")
...
# End of Hygge Project – Seq generator



# Ui.R for hygge project
bsCollapsePanel(
